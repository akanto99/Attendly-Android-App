...
class ManualCalculationNew extends StatefulWidget {
  final String slug;
  ...
  const ManualCalculationNew({super.key,
    required this.slug,
  ...
  @override
  State<ManualCalculationNew> createState() => _ManualCalculationNewState();
}

class _ManualCalculationNewState extends State<ManualCalculationNew>with WidgetsBindingObserver {
  ...
  Future<void> postData(List<String?> methods) async {
    setState(() {
      isLoading = true;
    });
    ...
    try {
      String apiUrl = '${AppUrl.baseUrl}/api/app/calculation/manual/staff/${widget.slug}';
      ...
      if (response.statusCode == 200) {
        ...
        if (response.body.contains('Success')) {
          ...
          Utils.flushBarErrorMessage("Server error.", context);
        }
      } else {
      ...
      child: Column(
        children: [
          ...
          Padding(
            ...
            child: Row(
              ...
              children: [
                ...
                Text("Manual Timesheet",style: TextStyle(fontSize: 20,fontWeight: FontWeight.bold),),
                GestureDetector(
                  onTap: () {
                    showDialog(
                      context: context,
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                      // color: AppColors.navColor,
                        ...
                        border: Border.all(
                            width: 0.1, color: AppColors.navColor
          ...
          GestureDetector(
            ...
            //   if (_dayTimes.isEmpty || _nightTimes.isEmpty
            //   // || _expenseId.isEmpty
            //   ) {
            //     Utils.flushBarErrorMessage("Fields must not be empty", context);
            //   }else{
            //     await postData(widget.methods);
            //   }
            // },
            onTap: () async {
              if (_hasDataInLists()) {
  ...
  Widget _buildDateDropdown(
      DateTime selectedDate, ValueChanged<DateTime> onChanged
      ) {
