...
import 'package:c9_app/Modules/StaffModule/StaffSettings/staffSettings.dart';
...
class NormalCalculationNew extends StatefulWidget {
  final String slug;
  ...
  @override
  State<NormalCalculationNew> createState() => _NormalCalculationNewState();
}

class _NormalCalculationNewState extends State<NormalCalculationNew> with WidgetsBindingObserver {
  ...
  Future<void> _NomalCalculationPost(List<String?> methods) async {
    setState(() {
      isLoading = true;
    });
    ...
    try {
      String apiUrl = '${AppUrl.baseUrl}/api/app/normal/calculation/store/${widget.slug}';
      print(apiUrl);
      ...
      // request.fields['end_date'] = _endDateController.text.toString();
      ...
      // request.fields['hour'] = totalShiftHours;
  ...
  void _calculateTimeDifference() {
    ...
    **/
    ...
    setState(() {
      ...
      // Calculate the total value by summing up nightShiftHours and dayShiftHours
  ...
  //       // Set next date to the next day
  //       DateTime selectedDate = DateTime.parse(_startSDateController.text);
  //       setState(() {
  //         nextDate = DateFormat('MM/dd/yyyy').format(selectedDate.add(Duration(days: 1)));
  //       });
  //     } else {
  //       // Reset next date
  //       setState(() {
  ...
  //     }
  //   } else {
  //     // Reset next date if any of the fields is empty
  //     setState(() {
  //       nextDate = '';
  //     });
  //   }
  ...
  Future<PeriodGetRates?> getRates() async {
    ...
    if (connectivityResult == ConnectivityResult.none || !hasInternet) {
      ...
      final String apiUrl = '${AppUrl.baseUrl}/api/app/get-staff-assigned-rates/${widget.clientslug}';
      print(apiUrl);
  ...
  Widget body() {
    ...
    final bool isExpanded = (dayShiftHours + nightShiftHours >= 6.0) || (TotalHours >= 6.0);
                              ...
                              GestureDetector(
                                onTap: () async {
                                  if (_startSDateController.text.isEmpty ||
                                    ...
                                    await _NomalCalculationPost(widget.methods);
                                  }
                                },
                                child: Container(
                ...
                return GestureDetector(
                  ...
                  onTap: () async {
                    ...
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (context) => StaffCurveNabBar(initialIndex: index),
                      ),
                    );
    print(index);
                    // Reset the tapped index
  ...
  void _clearFields() {
    setState(() {
      ...
      // _totalDayhourController.clear();
      // _totalnighthourController.clear();
    });
  }
}
