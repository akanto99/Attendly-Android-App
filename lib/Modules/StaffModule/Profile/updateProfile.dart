...
class _UpdateProfileState extends State<UpdateProfile> with WidgetsBindingObserver {
    ...
    try {
      ...
      if (dobText.isNotEmpty) {
        ...
        if (dobParts.length == 3) {
          String dobForSubmission = "${dobParts[2]}-${dobParts[1]}-${dobParts[0]}";
      ...
      if (_lasteyeController.text.isNotEmpty) {
        request.fields['last_eye_test'] =
            DateFormat('yyyy-MM-dd').format(DateFormat('dd-MM-yyyy').parse(_lasteyeController.text.toString()));
      }
      ...
      if (_selectedRole?.toLowerCase().contains('driver') ?? false) {
        ...
        if (!isCheckboxSelected) {
          ...
          if (cpcExpText.isNotEmpty) {
            ...
            if (cpcExpParts.length == 3) {
              String cpcExpSubmission = "${cpcExpParts[2]}-${cpcExpParts[1]}-${cpcExpParts[0]}";
        ...
        if (!isTechoCheckboxSelected) {
          ...
          if (tachoExpText.isNotEmpty) {
            ...
            if (tachoExpTextParts.length == 3) {
              String tachoExpSubmission = "${tachoExpTextParts[2]}-${tachoExpTextParts[1]}-${tachoExpTextParts[0]}";
        ...
        if (_dateofIssueDrivingController.text.isNotEmpty) {
          request.fields['driving_license_issue_date'] = DateFormat('yyyy-MM-dd')
                        ...
                        if (_selectedRole?.toLowerCase().contains('driver') ?? false) ...[
                          Row(
                            ...
                            children: [
                              ...
                              // doubleContainer(controller: _cpcNumberController, labelText: "CPC Number", keyboardType: TextInputType.name, prefixIcon: Icon(CupertinoIcons.number_square)),
                            ],
                          ),
                          SizedBox(
                            height: screenHeight * 0.013,
                            ...
                            child: Column(
                              children: [
                                DynamicDropdown(
                                  ...
                                  options: _dropdownOptions,
                                  selectedOption: _selected1 ?? 'No', // Provide a fallback
                                  onChanged: (newValue) {
                                    setState(() {
                                      _selected1 = newValue!;
                                    });
                                ...
                                if (_selected1 == 'Yes') ...[
                                  DynamicDropdown(
                                    ...
                                    options: _dropdownOptions,
                                    selectedOption: _selected2 ?? 'No', // Provide a fallback
                                    onChanged: (newValue) {
                                      setState(() {
                                        _selected2 = newValue!;
                                      });
                                ...
                                // if (_selected1 == 'Yes' && _selected2 == 'Yes' && _selected3 == 'Yes') ...[
                                // singleContainer(
                                //   labelText: 'Driving licence number.',
                                //   controller: _drivingLicenseNumberController,
                                //   keyboardType: TextInputType.name,
                                //   errorMessage: _validationErrors['driving_license_number'],
                                // ),
                                // SizedBox(
                                //   height: screenHeight * .013,
                                // ),],
            ...
            child: TextFormField(
              ...
              onTap: () async {
                ...
                if (_licExpiryController.text.isNotEmpty) {
                  try {
                    initialDate = DateFormat('dd-MM-yyyy').parse(_licExpiryController.text);
                  } catch (e) {
                    initialDate = DateTime.now();
                  }
                }
      ...
      child: TextFormField(
        ...
        onTap: isEnabled
          ...
          if (_cpcExpiryController.text.isNotEmpty) {
            try {
              initialDate = DateFormat('dd-MM-yyyy').parse(_cpcExpiryController.text);
            } catch (e) {
              initialDate = DateTime.now();
            }
          }
